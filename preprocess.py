...
def get_relation_dict(path):
    global relation2id
    with open(os.path.join(path, 'relations.dict'), encoding='utf-8') as fin:
        relation2id = dict()
        id_relation = dict()
        for line in fin:
          rid, relation = line.strip().split('\t')
          relation2id[relation] = rid
          id_relation[rid] = relation
    return  relation2id, id_relation

def get_entity_dict(path):
    global relation2id
    with open(os.path.join(path, 'entities.dict'), encoding='utf-8') as fin:
        relation2id = dict()
        id_relation = dict()
        for line in fin:
          rid, relation = line.strip().split('\t')
...
def preprocess_wn18rr(path):
    ...
    if not relation2id:
         get_relation_dict(os.path.dirname(path))
    lines = open(path, 'r', encoding='utf-8').readlines()
...
def _load_inductive_texts(path: str):
    global inductive_id2ent
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        assert len(fs) == 3, 'Invalid line: {}'.format(line.strip())
...
def _process_line_induct(line: str) -> dict:
    fs = line.strip().split('\t')
    assert len(fs) == 3, 'Expect 3 fields for {}'.format(line)
    head_id, relation, tail_id = fs[0], fs[1], fs[2]
    _, head, _ = inductive_id2ent[head_id]
    _, tail, _ = inductive_id2ent[tail_id]
...
inductive_id2ent=[]
def preprocess_inductive(path):
    if not wn18rr_id2ent:
        _load_inductive_texts('{}/wordnet-mlj12-definitions.txt'.format(os.path.dirname(path)))
    if not relation2id:
         get_relation_dict(os.path.dirname(path))
    lines = open(path, 'r', encoding='utf-8').readlines()
    pool = Pool(processes=args.workers)
...
def load_owe_des(path)
    global owe_id2ent, owe_id2desc
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        assert len(fs) == 2, 'Invalid line: {}'.format(line.strip())
